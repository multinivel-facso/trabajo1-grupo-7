...
b\) Pertenencia a pueblos originarios:

Por su parte, la variable pertenencia a pueblos originarios identifica si la persona se reconoce como perteneciente a algún pueblo originario. Es una variable categórica binaria o dummy, donde el valor 1 índica pertenencia y el 0 indica no pertenencia. Esta variable permite observar los porcentajes de pertenencia indígena y no indígena.
...
La variable dificultad de concentración mide el grado en que la persona declara tener dificultades para concentrarse o mantener la atención. Es una variable categórica ordinal, con una escala que va típicamente de 0 = no tiene dificultad a 3 = si presenta bastante dificultad.
...
Representa el nivel educacional más alto alcanzado por ambos de los padres del encuestado, como promedio. Es una variable categórica ordinal que va de valores 1 a 5, como es un promedio puede tener valores como 3.5. Valores más altos indican un mayor nivel de educación parental, lo que suele estar asociado positivamente con el nivel educacional alcanzado por los hijos.
...
Indica el nivel de acceso a conexión a internet en el hogar del encuestado. Esta variable va de valores entre 0 a 5, es una variable construida como escala sumativa, es decir los valores más altos tienen mas posibilidades de una mejor conexión o una conexión mas constante a internet.
...
La variable comuna identifica la unidad territorial en la que reside cada persona encuestada. Corresponde a una variable categórica nominal con 335 categorías posibles, una por cada comuna del país. Esta variable no tiene un orden jerárquico, por lo tanto es nominal. En este análisis la comuna se utiliza como nivel superior (nivel 2), permitiendo capturar la varianza educativa que puede atribuirse a factores territoriales o contextuales.
